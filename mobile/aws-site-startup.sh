...
if [ "$INSTANCE_ID" = "None" ]; then
	aws ec2 run-instances --image-id $IMAGE_ID --count 1 --instance-type m4.large --key-name its-bsa-prod-us-west-1-key-pair --security-group-ids sg-733f4b14 --subnet-id subnet-5207a536 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=test.usc.edu}]' 
	export INSTANCE_ID=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=test.usc.edu" --query Reservations[0].Instances[0].InstanceId --output text)
fi
